	...
	if (!str)
{
	...
	char* strCpy = new char[strlen(str) + 1]; // потому что string - const char
	strcpy(strCpy, str);
...
bool bigInt::saveToTextFile(const char* filename)
{
	...
	int len = strlen(str);
	int res = fwrite(str, sizeof(char), len, pfDestination);
...
{// возведение A в степень B по модулю modulus
	...
	bigInt* qw = new bigInt[count];
	bigInt* qw1 = new bigInt[count];
	l=B;
	int i = 0;
	while(l>=q)
	{
